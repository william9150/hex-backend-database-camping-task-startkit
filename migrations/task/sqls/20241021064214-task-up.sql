...
FROM 
  ...
  "CREDIT_PACKAGE" cp
WHERE 
  (u.email = 'wXlTq@hexschooltest.io' AND cp."name" = '14 堂組合包方案')
  or (u.email = 'wXlTq@hexschooltest.io' AND cp."name" = '21 堂組合包方案')
  or (u.email = 'richman@hexschooltest.io' AND cp."name" = '14 堂組合包方案');
...
FROM 
  ...
  "SKILL" s
WHERE 
  c.user_id = (SELECT u.id FROM "USER" u WHERE email = 'muscle@hexschooltest.io')
  AND s."name" = '瑜伽'
UNION
SELECT c.id AS coach_id, s.id AS skill_id
FROM 
  ...
  "SKILL" s
WHERE 
  c.user_id = (SELECT u.id FROM "USER" u WHERE email = 'starplatinum@hexschooltest.io')
  AND s."name" IN ('有氧運動', '復健訓練');
...
-- 5-4. 查詢：取得王小明所有的預約紀錄，包含取消預約的紀錄
 select cb.* from "COURSE_BOOKING" cb 
...
from 
	( select cp.user_id,sum(cp.purchased_credits) as total_credit
	from "CREDIT_PURCHASE" cp
	where cp.user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
	group by cp.user_id ) as "CREDIT_PURCHASE"
inner join 
	( select cb.user_id , count(*) as used_credit
	from "COURSE_BOOKING" cb 
	where cb.user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
	and status != '課程已取消'
	group by cb.user_id ) as "COURSE_BOOKING"
on "COURSE_BOOKING".user_id = "CREDIT_PURCHASE".user_id;
